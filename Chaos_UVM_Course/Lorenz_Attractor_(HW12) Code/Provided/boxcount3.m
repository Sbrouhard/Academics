function frac_dim = boxcount3(coordmat, maxstep, plotflag)
...
% Modified by Nick Allgaier, 4/27/11 to count boxes in 3D
...
glob_llz = min(coordmat(:,3)); % lower left corner of the initial box, z-coordinate
...
glob_urz = max(coordmat(:,3)); % upper right corner of the initial box, y-coordinate
glob_width = glob_urx - glob_llx; % width of the initial box
glob_length = glob_ury - glob_lly; % length of the initial box
glob_height = glob_urz - glob_llz; % height of the initial box
...
for step = 0:maxstep % loop over the subdivision steps 
  ...
  loc_length = glob_length/n_sds; % length of one box
  ...
  for sd_x = 1:n_sds % loop over local boxes in x-direction
    ...
    for sd_y = 1:n_sds % loop over local boxes in y-direction
      loc_lly = glob_lly + (sd_y - 1)*loc_length; % lower left corner, y-coordinate
      loc_ury = glob_lly + sd_y*loc_length; % upper right corner, y-coordinate
      % detect the set points inside the strip [loc_lly, loc_ury]:
      found_idy = find((found_y >= loc_lly) & (found_y < loc_ury)); % the indices
      found_z = coordmat(found_idx(found_idy), 3); % the z-coordinates
      for sd_z = 1:n_sds
            loc_llz = glob_llz + (sd_z - 1)*loc_height; % lower left corner, z-coordinate
            loc_urz = glob_llz + sd_z*loc_height; % upper right corner, z-coordinate
            % detect the set points inside the local box:
            inside_idx = find((found_z >= loc_llz) & (found_z < loc_urz));
            if (length(inside_idx) > 0)
            n_boxes = n_boxes + 1; % local box contains points of the set
            end
      end % loop over local boxes in z-direction
...
if (plotflag) 
  ...
  xlabel('n*ln(2)','fontsize',16);
  ylabel('ln(n\_boxes)','fontsize',16);
  set(gca,'fontsize',14)
end
